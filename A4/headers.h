...
typedef struct AdjList
{
    ...
    // set<pair<int, Action>> feed;
    priority_queue<pair<int, Action>, vector<pair<int, Action>>, greater<pair<int, Action>>> feed;
} AdjList;
...
class Graph
{
    ...
    Graph()
    {
        ...
        for(int i=0; i<MAX_NODES; i++)
        {
            ...
            nodelist[i].actionCount = vector<int>(3, 0);
            nodelist[i].nbrs = unordered_set<int>(MAX_NBRS); // calculated from the graph
            nodelist[i].degree = 0;
            nodelist[i].wall = queue<Action>(); // TODO: design size
            nodelist[i].feed = priority_queue<pair<int, Action>, vector<pair<int, Action>>, greater<pair<int, Action>>>(); // TODO: design size
        }
    }
