...
void* guest_routine(void* arg)
{
    ...
    try
    {
        for(;1;)
        {
            ...
            {
                sem_wait(&sem_stdcout); cout<<"Guest "<<idx <<" with priority "<<pr<<" waiting for clean-up to begin"<<endl; sem_post(&sem_stdcout);
                pthread_barrier_wait(&barr_guest);
            }

            pthread_mutex_lock(&mutex_guest[idx]);
            while(hotel.tot_occupancy == 2*N)
                pthread_cond_wait(&cond_guest, &mutex_guest[idx]);
            ...
            {
                sem_wait(&sem_stdcout); cout<<"Hotel full. Guest "<<idx <<" with priority "<<pr<<" looking for potential targets..."<<endl; sem_post(&sem_stdcout);
                sem_wait(&sem_hotel);
                ...
                {
                    sem_wait(&sem_stdcout); cout<<"Eviction target found at Room "<<evict_target_idx<<". Signalling to evict..."<<endl; sem_post(&sem_stdcout);
            ...
            else if (evict_status == 0) // Eviction signal
            {
                ...
                sem_wait(&sem_stdcout); cout<<"Guest "<<idx <<" with priority "<<pr<<" has been evicted from Room "<<current_room_idx<<endl; sem_post(&sem_stdcout);
            }   
