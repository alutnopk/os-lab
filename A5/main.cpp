...
int main(int argc, char **argv) // Legal argument range: 1 <= X < N < Y
{
    ...
    try
    {
        parse_input(argc, argv, X, N, Y);
    } // reads and validates command line inputs
    ...
    for (int i = 0; i < Y; i++)
    {
        ...
        if (ret)
        {
            cerr << "Failure in guest thread creation" << endl;
            return EXIT_FAILURE;
        }
    }
    for (int i = 0; i < X; i++)
    {
        ...
        if (ret)
        {
            cerr << "Failure in cleaner thread creation" << endl;
            return EXIT_FAILURE;
        }
    }
    ...
    {
        cerr << "Failure in semaphore initialization" << endl;
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < Y; i++)
    {
        ...
        if (ret)
        {
            cerr << "Failure in guest thread join" << endl;
            return EXIT_FAILURE;
        }
    }
    for (int i = 0; i < X; i++)
    {
        ...
        if (ret)
        {
            cerr << "Failure in cleaner thread join" << endl;
            return EXIT_FAILURE;
        }
    }
